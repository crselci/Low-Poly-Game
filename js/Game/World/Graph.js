...
export class Graph {
	...
	initGraph(grid) {
		...
		for (let j = 0; j < this.rows; j++) {
			for (let i = 0; i < this.cols; i++) {
				...
				else if (grid.length != 0 && grid[i][j] == 2) {
					node.type = TileNode.Type.Door;
				}
	...
	getRandomEmptyTile() {
		...
		while (this.nodes[index].type == TileNode.Type.Obstacle || this.nodes[index].type == TileNode.Type.Door) {
	...
	deleteTile(x, z) {
		this.nodes[z * this.cols + x].type = TileNode.Type.Ground;
	}
